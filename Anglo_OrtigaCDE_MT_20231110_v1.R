...
# Recalcula la normalización por área y por STD
shp$MEAN_1_per_area <- shp$MEAN_1 / (shp$AREA/10000)
shp$MEAN_1_normalized_STD <- shp$MEAN_1 / shp$STD

# Calcular el rango y definir los intervalos
range_MEAN_1 <- max(shp$MEAN_1, na.rm = TRUE) - min(shp$MEAN_1, na.rm = TRUE)
range_MEAN_1_per_area <- max(shp$MEAN_1_per_area, na.rm = TRUE) - min(shp$MEAN_1_per_area, na.rm = TRUE)
...
interval_MEAN_1_per_area <- range_MEAN_1_per_area / 10
...
breaks_MEAN_1_per_area <- seq(min(shp$MEAN_1_per_area, na.rm = TRUE), max(shp$MEAN_1_per_area, na.rm = TRUE), by = interval_MEAN_1_per_area)
...
shp$MEAN_1_per_area_cat <- cut(shp$MEAN_1_per_area, breaks = breaks_MEAN_1_per_area, include.lowest = TRUE, labels = FALSE)
...
labels_MEAN_1_per_area <- create_interval_labels(breaks_MEAN_1_per_area)
...
# Mejorar el histograma para MEAN_1
...
# Mejorar el estilo del mapa para MEAN_1 per Area
plot_shp_MEAN_1_per_area_cat <- ggplot(data = shp) +
  geom_sf(aes(fill = cut(MEAN_1_per_area, breaks = breaks_MEAN_1_per_area, include.lowest = TRUE, labels = labels_MEAN_1_per_area)), color = NA) +
  scale_fill_manual(values = colorRampPalette(c("green", "red"))(length(labels_MEAN_1_per_area)), labels = labels_MEAN_1_per_area) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    legend.direction = "horizontal",
...
# Mejorar el histograma para MEAN_1 per Area
hist_shp_MEAN_1_per_area <- ggplot(data = shp, aes(x = MEAN_1_per_area)) +
  geom_histogram(bins = 10, fill = "blue", color = "black") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 9, hjust = 0.5, face = "bold"),
...
hist_shp_MEAN_1_per_area

# Boxplot para MEAN_1 per Area
boxplot_shp_MEAN_1_per_area <- ggplot(data = shp, aes(y = MEAN_1_per_area)) +
  geom_boxplot(fill = "blue", color = "black") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 9, hjust = 0.5, face = "bold"),
...
boxplot_shp_MEAN_1_per_area

# Combinar el mapa y el histograma para MEAN_1 per Area
grid.arrange(plot_shp_MEAN_1_per_area_cat, hist_shp_MEAN_1_per_area, ncol = 2, widths = c(3, 2))

# Mejorar el estilo del mapa para MEAN_1 Normalizado por STD
...
# Mejorar el histograma para MEAN_1 Normalizado por STD
...
hist_shp_MEAN_1_normalized_STD

# Boxplot para MEAN_1 Normalizado por STD
boxplot_shp_MEAN_1_normalized_STD <- ggplot(data = shp, aes(y = MEAN_1_normalized_STD)) +
  geom_boxplot(fill = "blue", color = "black") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 9, hjust = 0.5, face = "bold"),
...
boxplot_shp_MEAN_1_normalized_STD


# Combinar el mapa y el histograma para MEAN_1 Normalizado por STD
